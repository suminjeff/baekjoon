...
def bfs(arr):
    ...
    visited = [[[inf for _ in range(5)] for _ in range(5)] for _ in range(5)]
    visited[0][0][0] = 0
    heap = [[0, 0, 0, 0]]
    while heap:
        ...
        for nh, nr, nc in [[h, r+1, c], [h, r-1, c], [h, r, c+1], [h, r, c-1], [h+1, r, c], [h-1, r, c]]:
            if 0 <= nh < 5 and 0 <= nr < 5 and 0 <= nc < 5 and arr[nh][nr][nc] == 1 and visited[nh][nr][nc] > nd:
                visited[nh][nr][nc] = nd
                heappush(heap, [nd, nh, nr, nc])
